...
<template>
    <TransitionGroup name="app_trans"><div id="container" key="home_container">
        ...
        <p>{{$t("settings.message.3")}}<input type="radio" ref="ipt4" @click="ipt4_select">{{$t("settings.message.4")}}<input type="radio" ref="ipt5" @click="ipt5_select">{{$t("settings.message.5")}}</p>
        <p>{{$t("settings.message.6")}}</p>
        <!--<a class="c" @click="this.$router.go(0)">{{$t("settings.message.7")}}</a>-->
    </div></TransitionGroup>
</template>
